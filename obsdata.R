...
startyr = 1992; endyr = 2009
nyears = length(startyr:endyr)
cal_rs = F # calculate rs with site met
if (cal_rs) {
  startyr = 1993; endyr = 2001  
  nyears = length(startyr:endyr)
  fl.02 = read.csv('~/Dropbox/DryDepEval/HarvardForest/flux/hf004-02-filled.csv') 
  ...
  fl.01$mix.ratio = mapply(mix_ratio, fl.01$ta.27.9m, fl.01$rh.27.9m, fl.01$p.amb, fl.01$ta.27.9m)
  ...
  fl.01 = merge(fl.02[,c('datetime','ustar')], fl.01, by = c('datetime')) # to merge ustar from hf004-02
  fl.01$ustar = fl.01$ustar/100
  fl.01$o3vd = mapply(v.d, fl.01$p.amb, fl.01$f.o3, fl.01$o3.mlb, fl.01$ta.27.9m) 
  fl.01 = fl.01[which(!is.na(fl.01$o3vd) & !is.infinite(fl.01$o3vd)),]
  hf.data = fl.01[c('datetime','ustar','year','doy','month','o3vd')]
...
### add filter ###
# (1) keep if ustar > 0.2 & !is.na(o3vd) 
hf.hrly = hf.data[which(!is.na(hf.data$o3vd) & !is.infinite(hf.data$o3vd) & (hf.data$ustar > 0.2)),]
...
if (cal_rs) {
  ...
  hf.hrly = hf.hrly[c('year','month','day','hour','o3vd')]
  hf.daily = aggregate(hf.hrly[,c('o3vd')], list(hf.hrly$day, hf.hrly$month, hf.hrly$year), mean, na.rm = T) # daily vd 24hr
}

hf.daily = aggregate(hf.hrly[,c('o3vd')], list(hf.hrly$day, hf.hrly$month, hf.hrly$year), mean, na.rm = T)
colnames(hf.daily) = c('day','month','year','o3vd')
# hf.daytime.hrly = hf.hrly[which(hf.hrly$hour >= 9 & hf.hrly$hour <= 15),]  # daytime daily vd: 9:00 ~ 15:00 
hf.daytime.hrly = hf.hrly[which(hf.hrly$hour >= 6 & hf.hrly$hour <= 18),]  # daytime daily vd: 6:00 ~ 18:00 
hf.day.daily =aggregate(hf.daytime.hrly[,c('o3vd')], list(hf.daytime.hrly$day, hf.daytime.hrly$month, hf.daytime.hrly$year), mean, na.rm = T)
colnames(hf.day.daily)[1:4] = c('day','month','year','o3vd')

# convert dataframes to arrays
hf.daily = hf.daily[which(hf.daily$o3vd > 0),] # keep daily vd values > 0
hf.dly.vd = array(NA, dim = c(nyears, 12, 30))
for (iyear in startyr:endyr) {
  for (imonth in 1:12) {
    for (iday in 1:30) {
      tmp = hf.daily[which(hf.daily$year == iyear & hf.daily$month == imonth & hf.daily$day == iday),]
      if (nrow(tmp)!=0) {
        hf.dly.vd[iyear-startyr+1, imonth, iday] = tmp$o3vd
        remove(tmp)
      }
    }
  }
}
hf.day.dly.vd = array(NA, dim = c(nyears, 12, 30))
for (iyear in startyr:endyr) {
  for (imonth in 1:12) {
    for (iday in 1:30) {
      tmp = hf.day.daily[which(hf.day.daily$year == iyear & hf.day.daily$month == imonth & hf.day.daily$day == iday),]
      if (nrow(tmp)!=0) {
        hf.day.dly.vd[iyear-startyr+1, imonth, iday] = tmp$o3vd
        remove(tmp)
      }
    }
  }
...
for (imonth in 1:12) {
  ...
  if (nrow(tmpvd) > 10) {
    ...
    hf.btstrp.day.avgmly[imonth, 2] = ci.results$bca[4]  # 95% confidence interval
    hf.btstrp.day.avgmly[imonth, 3] = ci.results$bca[5]
    
    # hf.btstrp.day.avgmly[imonth, 1] = mean(tmpboot)
    # hf.btstrp.day.avgmly[imonth, 2] = sd_trim(tmpboot, trim = 0.25, const = T) # sd
  }
}
...
for (ihour in 1:24) {
  ...
  if (nrow(tmpvd)>30) {
    ...
    hf.btstrp.summer.avghrly[ihour, 1] = mean(tmpboot)
  ...
  if (nrow(tmpvd)>10) {
    ...
    hf.btstrp.winter.avghrly[ihour, 1] = mean(tmpboot)
    hf.btstrp.winter.avghrly[ihour, 2] = sd_trim(tmpvd$o3vd, trim = 0.25, const = F)
  }
}
...
# mean+-3sd
mean.vd = mean(bf.hrly$o3vd) 
sd.vd = sd(bf.hrly$o3vd)
outlier.max = mean.vd + sd.vd * 3
outlier.min = mean.vd - sd.vd * 3
bf.hrly = bf.hrly[which(bf.hrly$o3vd < outlier.max & bf.hrly$o3vd > outlier.min),]
...
tmp = bf.hrly[which(bf.hrly$hour >= 6 & bf.hrly$hour <= 18),]  # daytime 6:00 - 18:00
...
for (iyear in 2008:2013) {
  for (imonth in 1:12) {
    for (iday in 1:30) {
      tmp = bf.vd.day.daily[which(bf.vd.day.daily$year == iyear & bf.vd.day.daily$month == imonth & bf.vd.day.daily$day == iday),]
      if (nrow(tmp) != 0) {
...
for (ihour in 1:24) {
  #summer
  tmpvd = bf.hrly[which(bf.hrly$hour == ihour-1 & (bf.hrly$month == 6 | bf.hrly$month == 7 | bf.hrly$month == 8 | bf.hrly$month == 9)),]
  if (nrow(tmpvd) > 10) {
    #tmp = tmpvd$o3vd
    tmp = tmpvd[(tmpvd$o3vd < quantile(tmpvd$o3vd, 0.8) & tmpvd$o3vd > quantile(tmpvd$o3vd, 0.2)),]
    tmp = tmp$o3vd
    results = boot(data = tmp, statistic = vd.mean, R = 1000)
    # bf.summer.avghrly[ihour,1] = mean(tmp, na.rm = T)
    bf.summer.avghrly[ihour,1] = results$t0
    bf.summer.avghrly[ihour,2] = sd(tmp,na.rm = T)
  }
  #winter
  tmpvd = bf.hrly[which(bf.hrly$hour == (ihour-1) & (bf.hrly$month == 12 | bf.hrly$month == 1 | bf.hrly$month == 2 | bf.hrly$month == 11 | bf.hrly$month == 3 | bf.hrly$month == 4)),]
  if (nrow(tmpvd) > 10) {
    tmp = tmpvd$o3vd
  ...
  #spring
  tmpvd = bf.hrly[which(bf.hrly$hour == ihour-1 & (bf.hrly$month == 3 | bf.hrly$month == 4 | bf.hrly$month == 5)),]
  if (nrow(tmpvd) > 10) {
    #tmp = tmpvd$o3vd
    tmp = tmpvd[(tmpvd$o3vd < quantile(tmpvd$o3vd, 0.8) & tmpvd$o3vd > quantile(tmpvd$o3vd, 0.2)),]
    tmp = tmp$o3vd
  ...
  #fall
  tmpvd = bf.hrly[which(bf.hrly$hour == ihour-1 & (bf.hrly$month == 9 | bf.hrly$month == 10 | bf.hrly$month == 11)),]
  if (nrow(tmpvd) > 10) {
    #tmp = tmpvd$o3vd
    tmp = tmpvd[(tmpvd$o3vd < quantile(tmpvd$o3vd, 0.8) & tmpvd$o3vd > quantile(tmpvd$o3vd, 0.2)),]
    tmp = tmp$o3vd
...
for (imonth in 1:12) {
  ...
  if (nrow(tmpvd) > 10) {
    ...
    hyy.btstrp.avgmonthly[imonth, 3] = sd_trim(tmpboot, trim = 0.25, const = F)
  }
...
for (ihour in 1:24) {  ## hour 0 ~ 23 
  ...
  if (nrow(tmpvd) > 10) {
    ...
    hyy.btstrp.summer.avghrly[ihour, 2] = sd_trim(tmpvd$o3vd, trim = 0.25, const = F)  # use sd
...
blo = blo[which(blo$o3vd < 0.05),] # 5 cm/s
# daily average 
...
for (imonth in 1:12) {
  ...
  if (nrow(tmpvd) > 10) {
    tmpboot = tmpvd$o3vd
    results = boot(data = tmpboot, statistic = vd.mean, R = 10000)
    ci.results = boot.ci(results, type = 'bca') 
  ...
  for (imonth in 1:12) {
    for (iday in 1:30) {
      for (ihour in 1:24) {
        ...
        hf9301.met$psi.H[iyear,imonth,iday,ihour] = psi.H(d = 16.8, L = hf9301.met$obk[iyear,imonth,iday,ihour], height = height)
        hf9301.met$ra[iyear,imonth,iday,ihour] = r.a(ustar = hf9301.met$ustar[iyear,imonth,iday,ihour], z0 = z0, L = hf9301.met$obk[iyear,imonth,iday,ihour], d = d, psi.H = hf9301.met$psi.H[iyear,imonth,iday,ihour], height = height)
  ...
  for (imonth in 1:12) {
    for (iday in 1:30) {
      for (ihour in 1:24) {
        ...
        fare10.met$psi.H[iyear,imonth,iday,ihour] = psi.H(d = d, L = fare10.met$obk[iyear,imonth,iday,ihour], height = height)
        fare10.met$ra[iyear,imonth,iday,ihour] = r.a(ustar = fare10.met$ustar[iyear,imonth,iday,ihour], z0 = z0, L = fare10.met$obk[iyear,imonth,iday,ihour], d = d, psi.H = fare10.met$psi.H[iyear,imonth,iday,ihour], height = height)
  ...
  for (imonth in 1:12) {
    for (iday in 1:30) {
      for (ihour in 1:24) {
        ...
        rannik12.met$psi.H[iyear,imonth,iday,ihour] = psi.H(d = d, L = rannik12.met$obk[iyear,imonth,iday,ihour], height = height)
        rannik12.met$ra[iyear,imonth,iday,ihour] = r.a(ustar = rannik12.met$ustar[iyear,imonth,iday,ihour], z0 = z0, L = rannik12.met$obk[iyear,imonth,iday,ihour], d = d, psi.H = rannik12.met$psi.H[iyear,imonth,iday,ihour], height = height)
...
bf08$psi.H = mapply(psi.H, d = d, L = bf08$obk, height = height)
bf08$ra = mapply(r.a, ustar = bf08$`U*(rot)`, z0 = z0, d = d, L = bf08$obk, psi.H =  bf08$psi.H, height = height) 
...
tmp$rs[tmp$rs < 0] = NA
hf9301.rs.day.mly = apply(tmp$rs[,,,7:19], c(2), mean, na.rm = T, trim = 0.05)
...
tmp = rannik12.met
tmp$rs[is.infinite(tmp$rs) | tmp$rs < 0 | tmp$rs > 10000] = NA
rannik12.rs.day.mly = apply(tmp$rs[,,,7:19], c(2), mean, na.rm = T, trim = 0.05)
rannik12.rs.mly = apply(tmp$rs, c(2), mean, na.rm = T, trim = 0.05)

# blodgett 
tmp = fare10.met
tmp$rs[is.infinite(tmp$rs) | tmp$rs < 0 | tmp$rs > 10000] = NA
fare10.rs.day.mly = apply(tmp$rs[,,,7:19], c(2), mean, na.rm = T, trim = 0.05)
